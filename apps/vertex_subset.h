#ifndef _VERTEX_SUBSET_H_
#define _VERTEX_SUBSET_H_
...
static int getWorkerNum() { return 0; }
...
public:
    ...
    VertexSubset(el_t e, uint64_t max_el_, bool all_ = false): all(all_), is_sparse(true), max_el(max_el_) {
        if (all) {
            is_sparse = false;
            return;
        }
        queue = new SlidingQueue<int32_t>(max_el); // max_el: num_vertices

        queue->push_back(e);
        queue->slide_window();
    }

    VertexSubset(bool *els, uint64_t len): all(false), is_sparse(false), max_el(len) {
  ...
  // just for keeping state like pushing past frontiers into a vector
    VertexSubset(const VertexSubset &other, bool copy_data): all(other.all), is_sparse(other.is_sparse), max_el(other.max_el) {
        if (copy_data) {
            if (all) {
                return;
            }
