...
struct EDGE_MAP_SPARSE {
    ...
    F f;
    bool output;
    EDGE_MAP_SPARSE(ntgraph &G_, VertexSubset &output_vs_, F f_, bool output_): G(G_), output_vs(output_vs_), f(f_), output(output_) {}
...
template <class F>
VertexSubset EdgeMapSparse(ntgraph &G, VertexSubset &vs, F f, bool output) {
    // printf("edge map sparse\n");
    vs.convert_to_sparse();
    VertexSubset output_vs = VertexSubset(vs, false); //copy_data = false
...
template <class F>
VertexSubset EdgeMapDense(ntgraph &G, VertexSubset &vs, F f, bool output) {
...
template <class F>
VertexSubset edgeMap(ntgraph &G, VertexSubset &vs, F f, bool output = true, uint32_t threshold = 20) {
    //vs.print();
